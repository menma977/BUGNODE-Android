...
class ChangeSecondaryPassword : AppCompatActivity() {
    ...
    private fun changePassword(){
        ...
        if(text_new_secondary_password.text.toString().isBlank())
            return Toast.makeText(applicationContext, "New password cannot be blank", Toast.LENGTH_SHORT).show()
        if(text_confirm_new_secondary_password.text.toString() != text_new_secondary_password.text.toString())
        ...
        body.addEncoded("old_password_key", text_secondary_password.text.toString())
        body.addEncoded("password_key", text_new_secondary_password.text.toString())
        Timer().schedule(100) {
            val res =
